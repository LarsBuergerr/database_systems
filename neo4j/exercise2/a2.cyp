...
MATCH (pr:Hoersaal {etage: 1})
WHERE pr.anzSitze >= 30
CALL
  gds.shortestPath.dijkstra.stream(
    "graph",
    {sourceNode: eingang, targetNode: pr, relationshipWeightProperty: 'dauer'}
  )
  YIELD path, totalCost
RETURN path, totalCost
...
CALL
  gds.shortestPath.dijkstra.stream(
    "graph_barrierefrei",
    {sourceNode: eingang, targetNode: raum, relationshipWeightProperty: 'dauer'}
  )
  YIELD path, totalCost
RETURN path, totalCost